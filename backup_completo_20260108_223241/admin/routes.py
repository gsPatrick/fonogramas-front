...
def importar_lote():
    ...
    if request.method == 'POST':
        ...
        if modo == 'validar':
            resultado_final = {
                'total_linhas': 0, 'linhas_validas': 0, 'linhas_com_erro': 0, 'erros': []
            }
            ...
            for arquivo in arquivos:
                ...
                # Adicionar nome do arquivo aos erros para identificar
                for invalido in res['lista_invalidos']:
                    erros = invalido.get('erros', [])
                    for erro in erros:
                        resultado_final['erros'].append({
                            'linha': f"{arquivo.filename}: {invalido.get('isrc', 'Linha?')}",
                            'erro': erro,
                            'dados': str(invalido)
                        })
            ...
            if not erros_gerais:
                flash(f'Importação concluída! Salvos: {total_salvos}, Atualizados: {total_atualizados}', 'success')
            else:
                flash(f'Importação parcial. Sucesso: {total_salvos}. Erros: {"; ".join(erros_gerais)}', 'warning')
...
def excluir_lote():
    ...
    if request.method == 'POST':
        ...
        if resultado['sucesso']:
            flash(f'{resultado["excluidos"]} fonogramas excluídos.', 'success')
        else:
            flash(f'Erro: {resultado["erro"]}', 'danger')
    
    return render_template('admin/lote/excluir.html')
